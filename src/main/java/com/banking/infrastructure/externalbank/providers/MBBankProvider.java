...
public class MBBankProvider implements ExternalBankProvider {
    ...
    @Value("${banking.provider.mb.base-url}")
    private String baseUrl;

    @Override
    public boolean validateAccount(String accountNumber) {
        log.info("MBBank: Validating account {}", accountNumber);
        return accountNumber != null && accountNumber.startsWith("011");
    }

    @Override
    public String executeTransfer(String toAccountNumber, BigDecimal amount, String currency, String idempotencyKey) {
        log.info("MBBank: Transfer {} {} to account {}", amount, currency, toAccountNumber);
        return "MB-" + java.util.UUID.randomUUID();
    }
